...
typedef struct test_hook_on_message_send_ctx
{
	eaf_sem_t*			sem;
}test_hook_on_message_send_ctx_t;

static test_hook_on_message_send_ctx_t s_test_hook_on_message_send_ctx;

static int _test_hook_on_message_send(uint32_t from, uint32_t to, eaf_msg_t* msg)
{
	EAF_SUPPRESS_UNUSED_VARIABLE(from, to, msg);

	eaf_sem_post(s_test_hook_on_message_send_ctx.sem);
	return 0;
}

TEST_FIXTURE_SETUP(eaf_hook)
{
	ASSERT_NE_PTR(s_test_hook_on_message_send_ctx.sem = eaf_sem_create(0), NULL);
	...
	hook.on_message_send = _test_hook_on_message_send;
	ASSERT_EQ_D32(eaf_inject(&hook, sizeof(hook)), 0);
}

TEST_FIXTURE_TEAREDOWN(eaf_hook)
{
	eaf_sem_destroy(s_test_hook_on_message_send_ctx.sem);
}

TEST_F(eaf_hook, on_message_send)
{
	...
	ASSERT_EQ_D32(eaf_sem_pend(s_test_hook_on_message_send_ctx.sem, 1 * 1000), 0);
