...
TEST(powerpack_string, apply)
{
	...
	{
		size_t token = 0;
		int ret = eaf_string_apply(quick_buffer, sizeof(quick_buffer), &token, source_string);
	...
	{
		size_t token = 0;
		int ret = eaf_string_apply(quick_buffer, 1, &token, source_string);
		ASSERT_EQ_D32(ret, strlen(source_string));
	...
	}
	{
		...
		size_t token = 0;
		int ret = eaf_string_apply(quick_buffer, 0, &token, source_string);
		ASSERT_EQ_D32(ret, strlen(source_string));
