...
TEST_F(powerpack_ringbuffer, init_exit)
{
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), 0);
	ASSERT_EQ_D32(counter.committed, 0);
	ASSERT_EQ_D32(counter.reading, 0);
	ASSERT_EQ_D32(counter.writing, 0);
...
TEST_F(powerpack_ringbuffer, consume_empty)
{
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), (size_t)0);
...
TEST_F(powerpack_ringbuffer, consume_when_reserve_not_committed)
{
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), 1);
	ASSERT_EQ_D32(counter.writing, 1);
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), (size_t)1);
	ASSERT_EQ_D32(counter.committed, (size_t)1);

	/* acquire read token must success */
	eaf_ringbuffer_token_t* read_token = eaf_ringbuffer_consume(rb);
	ASSERT_NE_PTR(read_token, NULL);
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), 1);
	ASSERT_EQ_D32(counter.reading, 1);
...
TEST_F(powerpack_ringbuffer, consume_when_reserve_not_committed2)
{
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), 2);
	ASSERT_EQ_D32(counter.writing, 2);
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), 2);
	ASSERT_EQ_D32(counter.writing, 1);
	ASSERT_EQ_D32(counter.committed, 1);
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), 2);
	ASSERT_EQ_D32(counter.committed, 2);
...
TEST_F(powerpack_ringbuffer, consume_after_reserve_10K)
{
	...
	for (i = 0; i < 10000; i++)
	{
		uint64_t data = i;
		{
			...
			ASSERT_EQ_D32(token->size.size, sizeof(uint64_t));
...
TEST_F(powerpack_ringbuffer, max_element_no_overwrite)
{
	...
	for (i = 0; i < number_of_elements; i++)
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), i + 1);
		ASSERT_EQ_D32(counter.writing, (size_t)1);
		ASSERT_EQ_D32(counter.committed, (size_t)i);

		memset(token->data, 0, sizeof(uint64_t));
		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);

		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), i + 1);
		ASSERT_EQ_D32(counter.committed, i + 1);
	}
	...
	/* because ring buffer is full, reserve must failed */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &counter), number_of_elements);
		ASSERT_EQ_D32(counter.committed, number_of_elements);
	}
...
TEST_F(powerpack_ringbuffer, reserve_consume_8bit_full)
{
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), 0);
	...
	for (;; token_counter++)
	{
		...
		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rb_counter), token_counter + 1);
		ASSERT_EQ_D32(rb_counter.writing, 1);
		ASSERT_EQ_U64(rb_counter.committed, token_counter);

		memcpy(token->data, &token_counter, sizeof(uint64_t));
		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);

		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rb_counter), token_counter + 1);
		ASSERT_EQ_U64(rb_counter.committed, token_counter + 1);
	}
	...
	for (i = 0; i < token_counter; i++)
	{
		...
		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rb_counter), token_counter - i);
		ASSERT_EQ_U64(rb_counter.committed, token_counter - i - 1);
		ASSERT_EQ_D32(rb_counter.reading, 1);

		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);

		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rb_counter), token_counter - i - 1);
		ASSERT_EQ_U64(rb_counter.committed, token_counter - i - 1);
	}
...
TEST_F(powerpack_ringbuffer, overwrite_generic)
{
	...
	for (i = 0; i < node_number; i++)
	{
		...
		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rb_counter), i + 1);
		ASSERT_EQ_D32(rb_counter.writing, 1);
		ASSERT_EQ_U64(rb_counter.committed, i);

		memcpy(token->data, &i, sizeof(uint64_t));
		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);

		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rb_counter), i + 1);
		ASSERT_EQ_U64(rb_counter.committed, i + 1);
	}
	...
	/* check oldest data */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), node_number);
		ASSERT_EQ_D32(rb_counter.committed, node_number - 1);
		ASSERT_EQ_D32(rb_counter.reading, 1);

		/* the first data should be 0 */
		uint64_t tmpval = 0;
		ASSERT_EQ_D32(token->size.size, sizeof(tmpval));
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), node_number);
		ASSERT_EQ_D32(rb_counter.committed, node_number);
	}
	...
	/* overwrite oldest data */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), node_number - 1);
		ASSERT_EQ_D32(rb_counter.committed, node_number - 2);
		ASSERT_EQ_D32(rb_counter.writing, 1);
	...
	/* check oldest data */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), node_number - 1);
		ASSERT_EQ_D32(rb_counter.committed, node_number - 2);
		ASSERT_EQ_D32(rb_counter.reading, (size_t)1);
...
TEST_F(powerpack_ringbuffer, overwrite_non_reserve)
{
	...
	eaf_ringbuffer_counter_t rb_counter;
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), (size_t)0);
	...
	for (i = 0; i < node_num; i++)
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), i + 1);
		ASSERT_EQ_D32(rb_counter.writing, i + 1);
	}
	...
	/* try to overwrite a node should failed */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), node_num);
		ASSERT_EQ_D32(rb_counter.writing, node_num);
	}
...
TEST_F(powerpack_ringbuffer, overwrite_only_one)
{
	...
	/* reserve 100 byte */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), 1);
		ASSERT_EQ_D32(rb_counter.writing, 1);

		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);

		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rb_counter), 1);
		ASSERT_EQ_D32(rb_counter.committed, 1);
	}
	...
	/* overwrite */
	{
		...
		ASSERT_NE_PTR(token, NULL);
		ASSERT_EQ_D32(token->size.size, 1);
...
TEST_F(powerpack_ringbuffer, write_discard_only_one)
{
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), 0);
	...
	/* acquire a write token and discard it */
	{
		...
		eaf_ringbuffer_token_t* token = eaf_ringbuffer_reserve(rb, sizeof(uint64_t), 0);
		ASSERT_NE_PTR(token, NULL);
		ASSERT_EQ_D32(token->size.size, sizeof(val));
		memcpy(token->data, &val, sizeof(val));

		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), 1);
		ASSERT_EQ_D32(rbc.writing, (size_t)1);
...
TEST_F(powerpack_ringbuffer, consume_discard_only_one)
{
	...
	const uint64_t tmpval = __LINE__;
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), 1);
		ASSERT_EQ_D32(rbc.writing, 1);

		memcpy(token->data, &tmpval, sizeof(tmpval));
		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);

		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), 1);
		ASSERT_EQ_D32(rbc.committed, 1);
	}
...
TEST_F(powerpack_ringbuffer, discard_older_consume)
{
	...
	for (;;)
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_ele + 1);
		ASSERT_EQ_D32(rbc.committed, number_of_ele);
		ASSERT_EQ_D32(rbc.writing, 1);

		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);

		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_ele + 1);
		ASSERT_EQ_D32(rbc.committed, number_of_ele + 1);
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_ele);
	ASSERT_EQ_D32(rbc.committed, number_of_ele - 1);
	ASSERT_EQ_D32(rbc.reading, (size_t)1);

	eaf_ringbuffer_token_t* token_2 = eaf_ringbuffer_consume(rb);
	ASSERT_NE_PTR(token_2, NULL);

	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_ele);
	ASSERT_EQ_D32(rbc.committed, number_of_ele - 2);
	ASSERT_EQ_D32(rbc.reading, (size_t)2);
...
TEST_F(powerpack_ringbuffer, force_discard_older_consume)
{
	...
	eaf_ringbuffer_counter_t rbc;
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), (size_t)0);
	...
	for (i = 0; i < number_of_elements; i++)
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), i + 1);
		ASSERT_EQ_D32(rbc.committed, i);
		ASSERT_EQ_D32(rbc.writing, (size_t)1);

		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), i + 1);
		ASSERT_EQ_D32(rbc.committed, i + 1);
	}
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_elements);
	ASSERT_EQ_D32(rbc.reading, (size_t)1);
	ASSERT_EQ_D32(rbc.committed, number_of_elements - 1);
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_elements);
	ASSERT_EQ_D32(rbc.reading, (size_t)2);
	ASSERT_EQ_D32(rbc.committed, number_of_elements - 2);
	...
	/* discard 1st token */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_elements - 1);
		ASSERT_EQ_D32(rbc.reading, (size_t)1);
		ASSERT_EQ_D32(rbc.committed, number_of_elements - 2);
	}
	...
	/* commit 2st token */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), number_of_elements - 2);
		ASSERT_EQ_D32(rbc.reading, (size_t)0);
		ASSERT_EQ_D32(rbc.committed, number_of_elements - 2);
	}
...
TEST_F(powerpack_ringbuffer, discard_middle)
{
	...
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), (size_t)1);
	ASSERT_EQ_D32(rbc.writing, (size_t)1);
	ASSERT_NE_PTR(token_1, (void*)NULL);

	eaf_ringbuffer_token_t* token_2 = eaf_ringbuffer_reserve(rb, sizeof(uint64_t), 0);
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), (size_t)2);
	ASSERT_EQ_D32(rbc.writing, (size_t)2);
	ASSERT_NE_PTR(token_2, (void*)NULL);

	eaf_ringbuffer_token_t* token_3 = eaf_ringbuffer_reserve(rb, sizeof(uint64_t), 0);
	ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), (size_t)3);
	ASSERT_EQ_D32(rbc.writing, (size_t)3);
	...
	/* write to 1st token */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), (size_t)3);
		ASSERT_EQ_D32(rbc.committed, (size_t)1);
		ASSERT_EQ_D32(rbc.writing, (size_t)2);
	}
	...
	/* write to 3st token */
	{
		...
		ASSERT_EQ_D32(eaf_ringbuffer_count(rb, &rbc), (size_t)3);
		ASSERT_EQ_D32(rbc.committed, (size_t)2);
		ASSERT_EQ_D32(rbc.writing, (size_t)1);
	}
	...
	eaf_ringbuffer_token_t* r_token_1 = NULL;
	{
		...
		ASSERT_EQ_D32(r_token_1->size.size, sizeof(uint64_t));
	...
	eaf_ringbuffer_token_t* r_token_2 = NULL;
	{
		...
		ASSERT_EQ_D32(r_token_2->size.size, sizeof(uint64_t));
...
TEST_F(powerpack_ringbuffer, discard_newest)
{
	...
	for (i = 0; i < number_of_elements - 1; i++)
	{
		...
		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rbc), i + 1);
		ASSERT_EQ_D32(rbc.writing, (size_t)1);
		ASSERT_EQ_D32(rbc.committed, (size_t)i);

		ASSERT_EQ_D32(eaf_ringbuffer_commit(rb, token, 0), 0);
		ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &rbc), i + 1);
		ASSERT_EQ_U64(rbc.committed, i + 1);
	}
...
TEST_F(powerpack_ringbuffer, counter_full)
{
	...
	for (i = 0; i < number_of_elements; i++)
	{
		...
		{
			eaf_ringbuffer_counter_t counter;
			ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &counter), i + 1);
			ASSERT_EQ_U64(counter.committed, i);
			ASSERT_EQ_U64(counter.reading, 0);
			ASSERT_EQ_U64(counter.writing, 1);
		}
		...
		/* check data */
		{
			eaf_ringbuffer_counter_t counter;
			ASSERT_EQ_U64(eaf_ringbuffer_count(rb, &counter), i + 1);
			ASSERT_EQ_U64(counter.committed, i + 1);
			ASSERT_EQ_U64(counter.reading, 0);
			ASSERT_EQ_U64(counter.writing, 0);
		}
	}
...
TEST_F(powerpack_ringbuffer, foreach)
{
	...
	{
		uint64_t tmp = 0;
		ASSERT_EQ_D32(eaf_ringbuffer_foreach(rb, _powerpack_ringbuffer_foreach, &tmp), number_of_elements);
	}
